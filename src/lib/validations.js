...
export const loginSchema = z.object({
  email: z.string().min(1, 'El correo electrónico es obligatorio').email('Ingresa un correo electrónico válido (ej. usuario@correo.com)'),
...
// 3. Validación de Jornada Multiapp (Horas + Minutos exactos o minutos totales)
export const dailyLogSchema = z.object({
  date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'La fecha debe tener formato AAAA-MM-DD (ej. 2026-03-15)'),
  grossIncome: z.coerce.number().min(0, 'El ingreso bruto debe ser mayor o igual a $0'),
  ...
  odometerKm: z.coerce.number().int().min(0, 'El odómetro debe ser un número entero mayor o igual a 0').default(0),
  hoursWorked: z.coerce.number().int().min(0, 'Las horas no pueden ser un número negativo').default(0),
  minutesWorked: z.coerce.number().int().min(0, 'Los minutos no pueden ser un número negativo').default(0),
...
export const expenseSchema = z.object({
  name: z.string().min(2, 'El concepto o nombre del gasto debe tener al menos 2 caracteres'),
  category: z.string().min(1, 'Debes seleccionar una categoría para el gasto'),
  type: z.enum(['fixed', 'one_time', 'installment'], {
    errorMap: () => ({ message: 'Tipo de gasto inválido (Fijo, Único o en Cuotas)' }),
  }),
  totalAmount: z.coerce.number().gt(0, 'El monto total del gasto debe ser mayor a $0'),
  installmentCount: z.coerce.number().int().min(1, 'La cantidad de cuotas debe ser al menos 1').max(60, 'El número máximo de cuotas es 60').default(1),
  startMonth: z.string().regex(/^\d{4}-\d{2}$/, 'El mes de inicio debe tener formato AAAA-MM (ej. 2026-03)'),
  dueDay: z.coerce.number().int().min(1, 'El día debe estar entre 1 y 31').max(31, 'El día debe estar entre 1 y 31').default(5),
  householdId: z.string().uuid().nullable().optional(),
  isShared: z.boolean().default(false),
  userSharePct: z.coerce.number().min(1, 'El porcentaje debe ser entre 1% y 100%').max(100, 'El porcentaje máximo permitido es 100%').default(100),
...
export const vehicleMaintenanceSchema = z.object({
  ...
  lastServiceKm: z.coerce.number().int().min(0, 'El kilometraje del último service debe ser mayor o igual a 0').default(0),
  ...
  category: z.string().min(1, 'Selecciona una categoría para el mantenimiento'),
...
export const serviceDoneSchema = z.object({
  ...
  serviceDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, 'La fecha del service debe ser AAAA-MM-DD'),
